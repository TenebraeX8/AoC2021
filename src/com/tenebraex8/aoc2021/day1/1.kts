...
lines.forEach { value->
    lastPair = when {
        lastPair.first == null -> lastPair.mapTo { Pair<Int?, Int?>(value, null) }
        lastPair.second == null -> lastPair.mapTo { Pair<Int?, Int?>(lastPair.first, value) }
        else -> {
            partialSums.add(lastPair.first!! + lastPair.second!! + value)
            lastPair.mapTo { Pair<Int?, Int?>(lastPair.second, value) }
        }
    }
}
