...
if __name__ == '__main__':
	...
	print([(lambda time: sum(reduce(lambda value, _: [value[(idx + 1) % len(value)] if idx != 6 else value[7] + value[0] for idx,_ in enumerate(list(value[1:]) + list([value[0]]))], list(range(time)), np.bincount([int(x) for x in read_input(6)[0].split(",")], minlength=9))))(stop) for stop in [80, 256]])
